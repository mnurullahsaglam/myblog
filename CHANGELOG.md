...
- `GET /api/v1/{resource}/schema` for every resource, returning the table and
  form definitions the panel already renders from. A client can build its lists
  and forms from these rather than hard-coding fifteen resources, and a field
  added in Laravel reaches it without a client release.
- The schemas inherit the access model: `schema()` already filters through
  `visibleColumns()` and `visibleFields()`, so a member never receives a field
  she may not see and a client cannot render a control for one. Read-only
  resources return a table schema and a null form.

## [0.13.1] - 2026-09-20

### Fixed

- The API emitted three different date formats. `IncomeResource` was written by
  hand while the other fourteen were generated from `attributesToArray()`, so a
  date-only column came back as `2026-09-15` from one and
  `2026-09-17T00:00:00.000000Z` from another, and timestamps differed again. A
  client decodes JSON with one date strategy, so this meant a special case per
  model forever. Every resource now emits dates as `Y-m-d` and timestamps as
  ISO 8601 with an offset, enforced by a test that compares the hand-written
  resource against a generated one.
...
- The invitation email read "X has invited you to X" whenever `APP_NAME` is the
  owner's own name. The inviter is now named only when it says something the
  sentence has not already said.

### Changed

- Local mail is documented as plain SMTP to Mailtrap's sandbox rather than an
  SDK, so nothing beyond the existing dependencies is needed to catch mail
  while developing.

### Note

- `QUEUE_CONNECTION=database` here, not `sync`. `InviteMail` implements
  `ShouldQueue`, so **invitations do not send unless a queue worker is
  running.** `composer run dev` runs `queue:listen`, so development is covered;
  any deployment must run a worker or invite mail will queue silently and never
  arrive. The panel's copyable link is what stops that being fatal.
