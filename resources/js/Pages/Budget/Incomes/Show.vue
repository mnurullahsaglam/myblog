...
<template>
    <AdminLayout title="Income">
        <template #actions>
            <Button
                label="Edit"
                icon="pi pi-pencil"
                size="small"
                @click="router.visit(route('admin.incomes.edit', recordId))"
            />
        </template>

        <div class="rounded-lg border border-surface-200 bg-surface-0 p-4 dark:border-[#272B35] dark:bg-[#15171C]">
            <dl class="grid gap-x-8 gap-y-4 sm:grid-cols-2">
                <div v-for="field in fields" :key="field.key">
                    <dt class="font-mono text-[11px] font-semibold uppercase tracking-[0.06em] text-surface-500">
                        {{ field.label }}
                    </dt>
                    <dd
                        class="mt-1 break-words text-[13px]"
                        :class="isMono(field) ? 'font-mono tabular-nums' : ''"
                    >{{ display(field) }}</dd>
                </div>
            </dl>
        </div>
