...
<template>
  ...
  >
    ...
    <p v-if="field.type === 'placeholder'" class="text-surface-500 font-mono text-[13px]">
      {{ placeholderValue ?? '—' }}
    </p>

    <div
      v-else-if="field.type === 'readonlyCode'"
      class="border-surface-200 bg-surface-100 flex items-center gap-2 rounded border px-3 py-2 dark:border-[#272B35] dark:bg-[#121317]"
    >
      <i class="pi pi-lock text-surface-500" style="font-size: 0.75rem" />
      <code class="text-surface-600 dark:text-surface-300 flex-1 truncate font-mono text-[13px]">
        {{ modelValue || '—' }}
      </code>
      <Button icon="pi pi-copy" text rounded size="small" severity="secondary" aria-label="Copy" @click="copyValue" />
    </div>
    ...
    <div v-else-if="field.type === 'markdown'" class="flex flex-col gap-2">
      ...
      <div :class="markdownMode === 'split' ? 'grid gap-3 lg:grid-cols-2' : ''">
        ...
        <article
          ...
          class="prose-sm border-surface-200 max-w-none overflow-auto rounded border p-3 text-[13px] dark:border-[#272B35]"
    ...
    <div v-else-if="field.type === 'file' || field.type === 'image'" class="flex flex-col gap-2">
      <img
        ...
        alt=""
        class="border-surface-200 h-24 w-24 rounded border object-cover dark:border-[#272B35]"
      />
      <p v-else-if="typeof modelValue === 'string' && modelValue" class="text-surface-500 font-mono text-[11px]">
        {{ modelValue }}
      </p>
