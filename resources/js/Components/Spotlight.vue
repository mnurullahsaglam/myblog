...
<template>
  <Dialog
    ...
    content-class="!p-0"
  >
    <div class="border-surface-200 flex items-center gap-2 border-b px-3 dark:border-[#272B35]">
      ...
      <kbd
        class="border-surface-200 text-surface-500 rounded border px-1.5 py-0.5 font-mono text-[10px] dark:border-[#272B35]"
        >esc</kbd
      >
    </div>

    <ul ref="listEl" class="max-h-80 overflow-y-auto p-2">
      <li v-if="!results.length" class="text-surface-500 px-3 py-8 text-center text-sm">
      ...
      <li
        ...
        :class="index === highlighted ? 'bg-primary/10 text-primary' : 'hover:bg-surface-100 dark:hover:bg-[#1A1D24]'"
        @mouseenter="highlighted = index"
        @click="select(target)"
      >
        <i :class="target.icon" style="font-size: 0.8rem" />
        <span class="flex-1 truncate">{{ target.label }}</span>
        <span class="text-surface-500 font-mono text-[11px]">{{ target.group }}</span>
      </li>
    </ul>
  </Dialog>
</template>
